...
public class IceLaserSkill : MonoBehaviour
{
  ...
  //  private WaitForSeconds skillCoolTime = new WaitForSeconds(skillDelayTime);
    ...
    [SerializeField] private GameObject iceLaserObject;
    ...
    private void IceLaserAttackRange()
    {
        ...
        if (enemyArray.Length > 0)
        {
            ...
            if (enemyList != null)
            {
                StartCoroutine(CreateIceLaser(enemyList));
            }
        }
    }


    private IEnumerator CreateIceLaser(List<Transform> enemyPositionList)
    {
        readyToFire = false;
        Vector2 dir;
        float angle;
        Quaternion angleAxis;
        SoundManager.GetInstacne().PlaySFXSound("IceLaser");
        ...
        for (int i = 0; i < projectileCount; i++)
        {
            ...
            GameObject iceLaser = Instantiate(iceLaserObject, transform.position, angleAxis);
            iceLaser.GetComponent<IceLaser>().SetLaserData(damage);
        }
